...
//----------------------------使用saveonepic--------------------//
class saveacepix extends savepapa
{
	//字段，事前进行设置的各个字段
	...
	//-----------------------------------构造函数---------------//
	function saveacepix($picdir)
	{
		//对相应的变量进行初始化
		$this->picdir		= $picdir;								//要访问的目录
		$this->bakdir		= "/data/www/ftpdir/acepix.bak";		//备份的目录
		$this->ifmod		= "1";									//是否审核
		$this->price		= "0";									//图片价格
		$this->ifedit		= "0";
		$this->ifstock		= "0";									//是否资料图
		$this->upuid		= "26468";								//摄影师id
		$this->upway		= "1";									//上传途径(0:历史数据1:ftp2前台3:后台)
		$this->upip			= "106.3.36.73";						//入库IP
		$this->penname		= "acepix";								//笔名
	...
	function cycle()
	{
		...
		foreach ($arr_pic as $kp => $findfile)
		{
			...
			//groupobj 成组标识，
			$this->info['groupobj'] = date("Ymd")."acepix".$this->info['catestr']."-".$this->picdir;
...
//需读取的文件夹
$filepath	= "/data/www/ftpdir/acepix";
