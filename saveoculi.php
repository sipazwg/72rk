...
//----------------------------使用saveonepic--------------------//
class saveoculi extends savepapa
{
	//字段，事前进行设置的各个字段
	...
	//-----------------------------------构造函数---------------//
	function saveoculi($picdir)
	{
		//对相应的变量进行初始化
		$this->picdir		= $picdir;								//要访问的目录
		$this->bakdir		= "/data/www/ftpdir/oculi.bak";		//备份的目录
		$this->ifmod		= "0";									//是否审核
		$this->price		= "0";									//图片价格
		$this->ifedit		= "0";
		$this->ifstock		= "0";									//是否资料图
		$this->upuid		= "109349";								//摄影师id
		$this->upway		= "1";									//上传途径(0:历史数据1:ftp2前台3:后台)
		$this->upip			= "106.3.36.73";						//入库IP
		$this->penname		= "oculi";								//笔名
		$this->plevel		= "2";									//图片的级别，sipausa与rex均为2级
		$this->zoneid		= "3";									//地区ID，1：国内，2：港奥台，3：国外
		$this->catestr		= 'SALL,GJXW,GJGS';
		$this->tpid			= "0";
		//继承父类中建立到数据库连接的代码
		parent::savepapa();
	}
	...
	function cycle()
	{
		...
		foreach ($arr_pic as $kp => $findfile)
		{
			...
			//groupobj 成组标识，
			$this->info['groupobj'] = date("Ymd")."oculi".$this->info['catestr']."-".$this->picdir;
			
			//单张入库
			
			if($this->info['gtitle'] == null ||$this->info['gtitle'] == ''){
				$this->info['gtitle'] = $this->info['caption']; 
			}
			
			//var_dump($this->info['gtitle']);
			//echo '12222222222222222222222222'
...
//需读取的文件夹
$filepath	= "/data/www/ftpdir/oculi";
