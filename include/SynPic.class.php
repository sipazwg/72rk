...
class SynPic
{
	...
	function syn($groupid,$photoid) 
	{
		...
		$sql_up = "update sipa_pic_cate set moddate='".$nowtime."'  where groupid='".$groupid."'";
		mysql_query($sql_up,$this->link);
		...
		if($pnum  != 1 )
		{
			...
			$this->updatepiccate($grow,$this->link);
		}
	...
	//操作临时表
	function updatepiccate($grow,$link)
	{
		//从临时表中清理数据
		$sql_del = "delete from sipa_pic_cate where groupid='".$grow['groupid']."'";
		...
		{
			$sql_ins = "insert into sipa_pic_cate(moddate,photoid,cid,groupid,ifmod,zoneid,ifstock,upuid) values('".$grow['moddate']."','".$grow['photoid']."','".$cid."','".$grow['groupid']."','".$grow['ifmod']."','".$grow['zoneid']."','".$grow['ifstock']."','".$grow['upuid']."')";
