...
//----------------------------使用saveonepic--------------------//
class savecordonpress extends savepapa
{
	//字段，事前进行设置的各个字段
	...
	//-----------------------------------构造函数---------------//
	function savecordonpress($picdir)
	{
		//对相应的变量进行初始化
		$this->picdir		= $picdir;								//要访问的目录
		$this->bakdir		= "/data/www/ftpdir/cordonpress.bak";		//备份的目录
		$this->ifmod		= "1";									//是否审核
		$this->price		= "0";									//图片价格
		$this->ifedit		= "0";
		$this->ifstock		= "0";									//是否资料图
		$this->upuid		= "109336";								//摄影师id
		$this->upway		= "1";									//上传途径(0:历史数据1:ftp2前台3:后台)
		$this->upip			= "106.3.36.74";						//入库IP
		$this->penname		= "cordonpress";								//笔名
	...
	function cycle()
	{
		...
		foreach ($arr_pic as $kp => $findfile)
		{
			...
			if(!$this->info['gtitle'])
			{
				$this->info['gtitle'] = substr($this->info['headline'],0,20);
			}
			
			if(!$this->info['gtitle'] && !$this->info['headline']){

				$this->info['gtitle'] = $this->info['title'];
			}
			
			//验证IPTC中是否含有频道的关键字
			if($this->info['category'] == 'SPO')
			{
				$this->info['catestr'] = 'SALL,XWTY';
			}else{
				$this->info['catestr']	= $this->catestr;
			}
			
			//groupobj 成组标识，
			$this->info['groupobj'] = date("Ymd")."-".$this->info['gtitle'];
...
//需读取的文件夹
$filepath	= "/data/www/ftpdir/cordonpress";
